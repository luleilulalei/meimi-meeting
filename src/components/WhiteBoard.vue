<template>
    <div class="white-board-div">
        ...
        <div class="color-group">
            <ul>
                <li
                    @click="clickColor($event)"
                    class="color-item"
                    v-for="(color, ind) in pencilColors"
                    ref="colorBtn"
                    :key="ind"
                    :style="color"
                ></li>
        ...
        <div class="tools">
            <button ref="brush" class="active" title="画笔" @click="brushClick">
                <i class="iconfont icon-qianbi"></i>
            </button>
            <button ref="eraser" title="橡皮擦" @click="eraserClick">
                <i class="iconfont icon-xiangpi"></i>
            </button>
            <button ref="clear" title="清空" @click="resetClick">
                <i class="iconfont icon-qingchu"></i>
            </button>
            <button ref="undo" title="撤销" @click="undoClick">
                <i class="iconfont icon-chexiao"></i>
            </button>
            <button ref="save" title="保存" @click="saveClick">
...
export default {
    ...
    mounted() {
        ...
        this.listenToUser(this.canvas, this.ctx);
    },
    methods: {
        autoSetSize(canvas) {
            this.canvasSetSize(canvas);
            window.onresize = () => this.canvasSetSize(canvas);
        },
        canvasSetSize(canvas) {
        ...
        listenToUser(canvas, ctx) {
            ...
            if (document.body.ontouchstart !== undefined) {
                canvas.ontouchstart = function (e) {
                    this.firstDot = ctx.getImageData(
                        0,
                        0,
                        canvas.width,
                        canvas.height
                    ); //在这里储存绘图表面
                    self.saveData(this.firstDot);
                    painting = true;
                    let x = e.touches[0].clientX;
                    let y = e.touches[0].clientY;
                    lastPoint = { x: x, y: y };
                    ctx.save();
                    self.drawCircle(x, y, 0);
                };
        ...
        brushClick() {
            ...
            this.$refs.brush.classList.add("active");
            this.$refs.eraser.classList.remove("active");
        },
        eraserClick() {
            this.clearStart = true;
            this.$refs.eraser.classList.add("active");
            this.$refs.brush.classList.remove("active");
        },
        resetClick() {
        ...
        undoClick() {
            let self = this;
            if(self.historyData.length < 1) return false;
            self.ctx.putImageData(self.historyData[self.historyData.length - 1], 0, 0);
            self.historyData.pop()
        },
        saveClick() {
        ...
        clickColor(e) {
            let btn = e.target;
            this.$refs.colorBtn.forEach(colorBtn => colorBtn.classList.remove('active'));                
            btn.classList.add("active");
            this.activeColor = btn.style.backgroundColor;
            this.ctx.fillStyle = this.activeColor;
            this.ctx.strokeStyle = this.activeColor;
        },
        saveData (data) {
            let self = this;
            (self.historyData.length === 10) && (self.historyData.shift());// 上限为储存10步，太多了怕挂掉
            self.historyData.push(data);
        }
    },
};
</script>
...
@font-face {
    ...
    src: url("//at.alicdn.com/t/font_584725_0nyjbeaxjw2ep14i.eot?t=1520519383959"); /* IE9*/
    src: url("//at.alicdn.com/t/font_584725_0nyjbeaxjw2ep14i.eot?t=1520519383959#iefix")
        ...
        /* IE6-IE8 */
            ...
            format("woff"),
        url("//at.alicdn.com/t/font_584725_0nyjbeaxjw2ep14i.ttf?t=1520519383959")
            format("truetype"),
        /* chrome, firefox, opera, Safari, Android, iOS 4.2+*/
            url("//at.alicdn.com/t/font_584725_0nyjbeaxjw2ep14i.svg?t=1520519383959#iconfont")
            format("svg"); /* iOS 4.1- */
}
